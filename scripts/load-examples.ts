 ...
 *
 * Usage:
 *   bun scripts/load-examples.ts                 # Load all datasets
 *   bun scripts/load-examples.ts --dataset imdb  # Load specific dataset
 *   bun scripts/load-examples.ts --generate      # Regenerate data
 *   bun scripts/load-examples.ts --verify        # Verify existing data
 *   bun scripts/load-examples.ts --list          # List available datasets
 *   bun scripts/load-examples.ts --help          # Show help
 */
...
interface DatasetConfig {
  ...
  generator: ((config: DatasetConfig) => Promise<void>) | null;
}
...
const DATASETS: Record<string, DatasetConfig> = {
  ...
  'onet-full': {
    ...
    generator: null, // Generated by scale-onet.ts
  },
  'unspsc': {
    name: 'UNSPSC',
  ...
  'unspsc-full': {
    ...
    generator: null, // Generated by scale-unspsc.ts
  },
};
...
async function getParquetInfo(filePath: string): Promise<ParquetInfo> {
  try {
    ...
    return {
      ...
      columns: metadata.schema.filter((s: { name: string }) => s.name !== 'schema').map((s: { name: string }) => s.name),
...
async function generateImdbData(config: DatasetConfig): Promise<void> {
...
async function generateOnetData(config: DatasetConfig): Promise<void> {
  console.log(`Generating ${config.name} data...`);
  const startTime = Date.now();
...
async function generateUnspscData(config: DatasetConfig): Promise<void> {
  console.log(`Generating ${config.name} data...`);
  const startTime = Date.now();
...
async function checkDatasetStatus(config: DatasetConfig): Promise<DatasetStatus> {
...
async function verifyDataset(datasetKey: string): Promise<boolean> {
...
async function loadDataset(datasetKey: string, options: LoadOptions = {}): Promise<boolean> {
  const config = DATASETS[datasetKey];
  if (!config) {
    console.error(`Unknown dataset: ${datasetKey}`);
    return false;
  ...
  if (!config.generator) {
    ...
    if (datasetKey.includes('full')) {
      console.log(`Use the corresponding scale script: bun scripts/scale-${datasetKey.split('-')[0]}.ts`);
    }
    return false;
  }
  ...
  try {
    ...
    console.error(`Error loading ${config.name}:`, (err as Error).message);
    return false;
  }
}

async function loadAllDatasets(options: LoadOptions = {}): Promise<void> {
...
Usage:
  bun scripts/load-examples.ts [command] [options]
...
Examples:
  ...
  bun scripts/load-examples.ts

  # Load specific dataset
  bun scripts/load-examples.ts --dataset imdb

  # Verify existing data
  bun scripts/load-examples.ts --verify

  # Force regeneration
  bun scripts/load-examples.ts --dataset imdb --force
