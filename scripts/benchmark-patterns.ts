...
interface BenchmarkResult {
  ...
  median: number
  rows: number
  rowGroupInfo: RowGroupStats | null
  statsHelp: string
  docRef: string
}
...
interface BenchmarkOptions {
  statsColumn?: string
  statsPredicate?: (min: unknown, max: unknown) => boolean
  useCase?: string
  docRef?: string
  statsHelp?: string
...
async function getRowGroupStats(file: FileHandle['file'], columnName: string): Promise<Array<{ num_values: number; min: unknown; max: unknown }> | null> {
  const meta = await parquetMetadataAsync(file)
  const colIndex = meta.schema.findIndex((c: { name: string }) => c.name === columnName) - 1
  if (colIndex < 0) return null

  return meta.row_groups.map((rg: { columns: Array<{ meta_data?: { num_values?: number; statistics?: { min_value?: unknown; max_value?: unknown } } }> }) => {
...
async function countRowGroupsWithStats(
  file: FileHandle['file'],
  columnName: string,
  predicate: (min: unknown, max: unknown) => boolean
): Promise<RowGroupStats> {
  const stats = await getRowGroupStats(file, columnName)
  if (!stats) return { total: 0, matching: 0, skipped: 0, skipRate: '0%' }
...
async function benchmark(
  ...
  const times: number[] = []
  let result: unknown[]
  let rowGroupInfo: RowGroupStats | null = null
...
async function benchmarkIMDBPatterns(): Promise<void> {
  console.log('\n' + '='.repeat(100))
  ...
  await benchmark('IMDB', 'P1: Movies by type + high rating (genre browse sim)', async () => {
    ...
    statsPredicate: (min, max) => (max as number) >= 7.5,
  ...
  await benchmark('IMDB', 'P2: New releases (startYear=2024)', async () => {
    ...
    statsPredicate: (min, max) => (min as number) <= 2024 && (max as number) >= 2024,
  ...
  await benchmark('IMDB', 'P3: Decade collection (1980s movies)', async () => {
    ...
    file: titles.file,
    statsColumn: '$index_startYear',
    statsPredicate: (min, max) => (min as number) < 1990 && (max as number) >= 1980,
  ...
  await benchmark('IMDB', 'P4: Popular content (numVotes >= 10000)', async () => {
    ...
    statsPredicate: (min, max) => (max as number) >= 10000,
  ...
  await benchmark('IMDB', 'P6: Top-rated (rating >= 8.5, votes >= 1000)', async () => {
    ...
    file: titles.file,
    statsColumn: '$index_averageRating',
    statsPredicate: (min, max) => (max as number) >= 8.5,
  ...
  await benchmark('IMDB', 'P13: Six degrees step 1 - person filmography', async () => {
    ...
    statsPredicate: (min, max) => (min as string) <= 'nm0000001' && (max as string) >= 'nm0000001',
  ...
  await benchmark('IMDB', 'P13: Six degrees step 2 - cast of a movie', async () => {
    ...
    statsPredicate: (min, max) => (min as string) <= 'tt0050000' && (max as string) >= 'tt0050000',
  ...
  await benchmark('IMDB', 'P14: Director-actor pairs (filter directors)', async () => {
    ...
    statsPredicate: (min, max) => (min as string) <= 'director' && (max as string) >= 'director',
  ...
  await benchmark('IMDB', 'P11: Type distribution (full scan)', async () => {
    const rows = await parquetQuery({
      ...
      compressors
    }) as Array<{ $index_titleType: string }>
    // Group by type
  ...
  await benchmark('IMDB', 'P12: Year-over-year 2010s', async () => {
    const rows = await parquetQuery({
      ...
      compressors
    }) as Array<{ $index_startYear: number }>
    // Group by year
    ...
    file: titles.file,
    statsColumn: '$index_startYear',
    statsPredicate: (min, max) => (min as number) <= 2019 && (max as number) >= 2010,
...
async function benchmarkONETPatterns(): Promise<void> {
  console.log('\n' + '='.repeat(100))
  ...
  console.log(`Columns (occ-skills): ${osMeta.schema.slice(1).map((c: { name: string }) => c.name).join(', ')}\n`)
  ...
  await benchmark('O*NET', 'Q1: Job Zone filter (jobZone=4)', async () => {
    ...
    statsPredicate: (min, max) => (min as number) <= 4 && (max as number) >= 4,
  ...
  await benchmark('O*NET', 'Q3: Occupations by job zone range (1-2)', async () => {
    ...
    file: occupations.file,
    statsColumn: '$index_jobZone',
    statsPredicate: (min, max) => (min as number) <= 2,
  ...
  await benchmark('O*NET', 'Q5: Skills importance >= 4.0', async () => {
    ...
    statsPredicate: (min, max) => (max as number) >= 4.0,
  ...
  await benchmark('O*NET', 'Q8: Skills for occupation 15-1252.00', async () => {
    ...
    statsPredicate: (min, max) => (min as string) <= '15-1252.00' && (max as string) >= '15-1252.00',
  ...
  await benchmark('O*NET', 'Q11: Occupations requiring skill 2.B.2.i', async () => {
    ...
    statsPredicate: (min, max) => (min as string) <= '2.B.2.i' && (max as string) >= '2.B.2.i',
  ...
  await benchmark('O*NET', 'Q14: Skill demand (all ratings for skill)', async () => {
    const rows = await parquetQuery({
      ...
      compressors
    }) as Array<{ $index_importance?: number }>
...
async function benchmarkUNSPSCPatterns(): Promise<void> {
  console.log('\n' + '='.repeat(100))
  ...
  console.log(`Columns (commodities): ${commMeta.schema.slice(1).map((c: { name: string }) => c.name).join(', ')}\n`)
  ...
  await benchmark('UNSPSC', 'P1.3: Segment filter (segmentCode=43)', async () => {
    ...
    statsPredicate: (min, max) => (min as string) <= '43' && (max as string) >= '43',
  ...
  await benchmark('UNSPSC', 'P2.2: Family drill-down (familyCode=4310)', async () => {
    ...
    statsPredicate: (min, max) => (min as string) <= '4310' && (max as string) >= '4310',
  ...
  await benchmark('UNSPSC', 'P4.1: Class filter (classCode=431015)', async () => {
    ...
    statsPredicate: (min, max) => (min as string) <= '431015' && (max as string) >= '431015',
  ...
  await benchmark('UNSPSC', 'P3.2: Multi-segment (43, 44)', async () => {
    const rows = await parquetQuery({
      file: commodities.file,
      compressors
    }) as Array<{ $index_segmentCode?: string }>
...
async function benchmarkStatsComparison(): Promise<void> {
  console.log('\n' + '='.repeat(100))
  ...
  const ratingScanResult = await benchmark('Compare', 'Rating >= 8.0: WITHOUT filter (full scan)', async () => {
    const rows = await parquetQuery({
      file: titles.file,
      compressors
    }) as Array<{ $index_averageRating: number }>
  ...
  const famScanResult = await benchmark('Compare', 'Family 4310: WITHOUT filter (full scan)', async () => {
    const rows = await parquetQuery({
      file: commodities.file,
      compressors
    }) as Array<{ $index_familyCode: string }>
...
function printSummaryReport(): void {
  ...
  const byStatsHelp: Record<string, BenchmarkResult[]> = {
    'Yes': [],
    'Partial': [],
    'No': [],
    'N/A': [],
