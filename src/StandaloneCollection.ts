/**
 * StandaloneCollection - Standalone in-memory Collection for testing and development
 *
 ...
 *
 * When to use StandaloneCollection vs ParqueDB:
 *
 * USE StandaloneCollection FOR:
 ...
 * - Quick scripts or one-off data processing
 *
 * USE ParqueDB FOR:
 * - Production applications requiring persistent storage
 * - Applications using storage backends (MemoryBackend, FsBackend, R2Backend, etc.)
 * - Multi-tenant or concurrent workloads
 * - Any scenario requiring data durability
 *
 * Key differences from ParqueDB.collection():
 * - Data is stored in module-level global Maps (shared across all StandaloneCollection instances)
 ...
 *
 * @example
 * // For testing (uses StandaloneCollection)
 * import { StandaloneCollection, clearGlobalStorage } from 'parquedb'
 * const posts = new StandaloneCollection<Post>('posts')
 ...
 * StandaloneCollection - Standalone in-memory collection for testing and development
 *
 * This class provides a MongoDB-style API for a specific entity type/namespace,
 * but stores data in module-level global Maps rather than a storage backend.
 *
 * @see StandaloneCollection module JSDoc for usage guidance
 *
 * @example
 * // For testing
 * import { StandaloneCollection, clearGlobalStorage } from 'parquedb'
 ...
 * clearGlobalStorage() // Clean up
 */
export class StandaloneCollection<T extends EntityData = EntityData> {
...
/**
 * @deprecated Use StandaloneCollection instead. Collection is an alias kept for backwards compatibility.
 */
export const Collection = StandaloneCollection
