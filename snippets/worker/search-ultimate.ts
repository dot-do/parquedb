/**
 * Ultimate Snippet Search Worker (v6)
 *
 * All features from v5 PLUS:
 * - Wildcards: engin* matches engineer, engineering
 * - Field boosting: boost=title:2,description:1
 * - Autocomplete: /suggest?q=eng → ["engineer", "engineering"]
 * - Field projection: fields=title,year (reduce payload)
 * - Distinct: distinct=titleType (unique values only)
 * - Explain mode: explain=true (show scoring breakdown)
 * - Snippet extraction: snippets=true (context around matches)
 * - More operators: AND, OR explicit support
 *
 * Bundle target: <10KB gzipped (still 22KB headroom)
 */
...
// Search Engine
async function search(ds: string, pq: ParsedQuery, boosts: Record<string, number>, env: Env): Promise<{
  indices: number[]; scores: Map<number, number>; matched: Set<string>; explains: Map<number, string[]>
}> {
  ...
  if (!allTerms.length && !pq.phrases.length) return { indices: [], scores, matched, explains }
  ...
  return { indices: sorted, scores, matched, explains }
}
...
function computeStats(docs: Record<string, unknown>[], fields: string[]): Record<string, { min: number; max: number; avg: number }> {
  ...
  for (const f of fields) {
    ...
    if (vals.length) r[f] = { min: Math.min(...vals), max: Math.max(...vals), avg: Math.round(vals.reduce((a, b) => a + b, 0) / vals.length * 10) / 10 }
  }
  return r
}
...
async function fetchDocs(
  ...
  facetFields: string[], statFields: string[], distinctField: string | null
): Promise<{ docs: Record<string, unknown>[]; total: number; facets?: Record<string, { value: string; count: number }[]>; stats?: Record<string, { min: number; max: number; avg: number }> }> {
  const needsAll = sort.length > 0 || filters.range.length > 0 || filters.exists.length > 0 || facetFields.length > 0 || statFields.length > 0 || distinctField
...
async function handleSearch(ds: string, p: URLSearchParams, env: Env): Promise<Response> {
  ...
  if (pq.should.length || pq.must.length || pq.wildcards.length || pq.phrases.length) {
    ...
    indices = result.indices; matched = result.matched; explains = result.explains
  } else {
  ...
  const { docs, total, facets, stats } = await fetchDocs(ds, indices, meta, env, limit, offset, sort, filters, facetFields, statFields, distinctField)
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    for (const pre of ['/search-v6', '/search-v5', '/search-v4', '/search-v3', '/search-v2', '/search']) {
    ...
    try {
      if (path === '/health') {
        return Response.json({
          ok: true, version: 6,
          features: ['fts', 'stemming', 'prefix', 'fuzzy', 'wildcards', 'synonyms', 'negation', 'phrase', 'filters', 'range', 'exists', 'not', 'sort', 'multi-sort', 'facets', 'stats', 'highlight', 'snippets', 'explain', 'fields', 'distinct', 'boost', 'suggest'],
        })
      }
      ...
      if (path === '/' || path === '') {
        return Response.json({
          name: 'ParqueDB Search (Ultimate v6)', version: 6,
          features: ['fts', 'stemming', 'prefix', 'fuzzy~', 'wildcards*', 'synonyms', 'negation -', 'phrase ""', 'filters', 'range', 'exists', 'not', 'sort', 'facets', 'stats', 'highlight', 'snippets', 'explain', 'fields', 'distinct', 'boost', 'suggest'],
          endpoints: {
          ...
          examples: [
            '/imdb?q=engin*&type=movie (wildcard)',
            '/imdb?q=love&explain=true (scoring breakdown)',
            '/imdb?q=matrix&snippets=true (context snippets)',
            '/imdb?q=love&fields=primaryTitle,startYear (projection)',
            '/imdb?type=movie&distinct=startYear (unique years)',
            '/suggest/imdb?q=mat (autocomplete)',
          ],
        })
      }
